...
public record class ScrFunction : IExportedSymbol
{
    ...
    private string GetParametersString()
    {
        ...
        var calledOn = IsBuiltIn ? null : Overloads.First().CalledOn;
    ...
    /// <summary>
    /// Whether this function's namespace can be skipped when calling it.
    /// This applies to:
    /// - Built-in API functions (sys namespace can be omitted)
    /// - Functions in the current script's namespace
    /// Note: This is broader than <see cref="IsBuiltIn"/> - it's about namespace resolution,
    /// not the origin of the function.
    /// </summary>
    ...
    /// <summary>
    /// Whether this function is a built-in API function (loaded from code API JSON).
    /// Built-in functions are autogenerated from the game's code and marked with the "autogenerated" flag.
    /// Note: This is different from <see cref="Implicit"/> - a script-defined function can be Implicit
    /// (when in the current namespace) but not IsBuiltIn. All built-in functions are Implicit, but not
    /// all Implicit functions are built-in.
    /// </summary>
