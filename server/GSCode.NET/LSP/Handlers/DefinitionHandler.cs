...
internal class DefinitionHandler : DefinitionHandlerBase
{
    ...
    public override async Task<LocationOrLocationLinks> Handle(DefinitionParams request, CancellationToken cancellationToken)
    {
        ...
        // If it's a builtin autogenerated API function, do not return a file location
        var api = ScriptAnalyserData.GetShared(script.LanguageId);
        if (api is not null)
        {
            var apiFn = api.GetApiFunction(name);
            if (apiFn is not null && apiFn.IsAutogenerated)
            {
                sw.Stop();
                _logger.LogInformation("Definition finished in {ElapsedMs} ms: autogenerated API {name}", sw.ElapsedMilliseconds, name);
                return new LocationOrLocationLinks();
            }
        }
