...
internal ref partial struct ReachingDefinitionsAnalyser(List<Tuple<ScrFunction, ControlFlowGraph>> functionGraphs, List<Tuple<ScrClass, List<ControlFlowGraph>>> classGraphs, ParserIntelliSense sense, Dictionary<string, IExportedSymbol> exportedSymbolTable, ScriptAnalyserData? apiData = null, string? currentNamespace = null, HashSet<string>? knownNamespaces = null, string? fileName = null)
{
        ...
        SymbolFlags flags)
    {
        ...
        if (function.Overloads is null || function.Overloads.Count == 0) return;
        if (!flags.HasFlag(SymbolFlags.BuiltIn)) return;  // only check API functions
        ...
        if (matchedOverload is null) return;  // no count-matching overload; count validator handles this

        // Check each argument against expected type
        for (int i = 0; i < Math.Min(argTypes.Length, matchedOverload.Parameters.Count); i++)
        {
            var param = matchedOverload.Parameters[i];
            if (param.Type is null) continue;  // no type info for this param

            ScrData expectedType = ScrData.FromApiType(param.Type);
            if (expectedType.IsAny()) continue;  // "any" accepts everything
            ...
            if (actualType.TypeUnknown()) continue;  // can't validate unknown types
    ...
    private static ScrFunctionOverload? FindBestOverload(List<ScrFunctionOverload> overloads, ScrData[] argTypes)
    {
        ...
        {
            if (overload?.Parameters is null) continue;
            int minArgs = overload.Parameters.Count(p => p.Mandatory == true);
            int maxArgs = overload.Parameters.Count;
            bool hasVararg = overload.Vararg;
            ...
            if (hasVararg)
            {
                if (argTypes.Length < minArgs) continue;
            }
            else
            {
                if (argTypes.Length < minArgs || argTypes.Length > maxArgs) continue;
            }
            ...
            for (int i = 0; i < Math.Min(argTypes.Length, overload.Parameters.Count); i++)
            {
                ...
                if (param.Type is null) { score++; continue; }

                ScrData expectedType = ScrData.FromApiType(param.Type);
                if (expectedType.IsAny()) { score++; continue; }
        ...
        SymbolFlags flags)
    {
        if (function is null) return;
        if (function.Overloads is null || function.Overloads.Count == 0) return;
        if (!flags.HasFlag(SymbolFlags.BuiltIn)) return;
        ...
        if (matchedOverload is null) return;

        int idx = 0;
        foreach (ExprNode? argNode in methodCall.Arguments.Arguments)
        {
            if (idx >= matchedOverload.Parameters.Count) break;
            var param = matchedOverload.Parameters[idx];
            if (param.Type is null || argNode is null) { idx++; continue; }

            ScrData expectedType = ScrData.FromApiType(param.Type);
            if (expectedType.IsAny()) { idx++; continue; }
            ...
            if (actualType.TypeUnknown()) { idx++; continue; }
