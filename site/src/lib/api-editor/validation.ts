...
export function validateFunction(fn: ScrFunction): string[] {
	...
	const isProcessed = fn.flags.includes('processed');

	errors.push(...validateName(fn));
	errors.push(...validateFlags(fn));
	errors.push(...validateConfidence(fn, isProcessed));
...
function validateFlags(fn: ScrFunction): string[] {
	const errors: string[] = [];
	const exclusive = ['autogenerated', 'verified', 'processed'].filter((f) => fn.flags.includes(f));
	if (exclusive.length > 1) {
		errors.push(`Flags: ${exclusive.join(', ')} are mutually exclusive.`);
	}
	return errors;
}

function validateConfidence(fn: ScrFunction, isProcessed: boolean): string[] {
	if (!isProcessed) return [];
	const errors: string[] = [];
	if (!fn.confidence) {
		errors.push('Confidence: required for processed entries.');
	}
	return errors;
}
